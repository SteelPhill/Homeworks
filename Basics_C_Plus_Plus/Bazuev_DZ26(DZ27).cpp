...
MenuItems Menu()
{
	...
	MenuItems choice = (MenuItems)CinValueInRange<int>(FirstPoint, LastPoint);
	cout << endl;
...
void ShowOneBook(Book& book, const int indexSelectedBook)
{
	cout << "Книга №" << indexSelectedBook + 1 << endl;
...
void EditName(Book& book)
{
	...
	book.Name[j++] = toupper(book.Name[i++]);

	while (i < book.Name.size())
		if (isspace((unsigned char)book.Name[i - 1]) && isspace((unsigned char)book.Name[i]))
			i++;
		else if (isspace((unsigned char)book.Name[i - 1]) && isalpha((unsigned char)book.Name[i]))
			book.Name[j++] = book.Name[i++];
		else
			book.Name[j++] = tolower(book.Name[i++]);
...
void EditWriter(Book& book)
{
	...
	book.Writer[j++] = toupper(book.Writer[i++]);

	while (i < book.Writer.size())
		if (isspace((unsigned char)book.Writer[i - 1]) && isspace((unsigned char)book.Writer[i]))
			i++;
		else if (isspace((unsigned char)book.Writer[i - 1]) && isalpha((unsigned char)book.Writer[i]))
			book.Writer[j++] = toupper(book.Writer[i++]);
		else
			book.Writer[j++] = tolower(book.Writer[i++]);

	book.Writer.resize(j);
}
...
void EditPublisher(Book& book)
{
	...
	book.Publisher[j++] = toupper(book.Publisher[i++]);

	while (i < book.Publisher.size())
		if (isspace((unsigned char)book.Publisher[i - 1]) && isspace((unsigned char)book.Publisher[i]))
			i++;
		else if (isspace((unsigned char)book.Publisher[i - 1]) && isalpha((unsigned char)book.Publisher[i]))
			book.Publisher[j++] = book.Publisher[i++];
		else
			book.Publisher[j++] = tolower(book.Publisher[i++]);

	book.Publisher.resize(j);
}
...
void EditGenre(Book& book)
{
	...
	book.Genre[j++] = toupper(book.Genre[i++]);

	while (i < book.Genre.size())
		if (isspace((unsigned char)book.Genre[i - 1]) && isspace((unsigned char)book.Genre[i]))
			i++;
		else if (isspace((unsigned char)book.Genre[i - 1]) && isalpha((unsigned char)book.Genre[i]))
			book.Genre[j++] = book.Genre[i++];
		else
			book.Genre[j++] = tolower(book.Genre[i++]);
...
void EditingBooks(Book* library, const int librarySize)
{
	...
	do
	{
		...
		ShowOneBook(library[selectedBook - 1], selectedBook - 1);
...
bool IsStringsEqualWithoutSpaces(string& bookName, string& searchName)
{
	...
	string bookNameWithoutSpaces;
	string searchNameWithoutSpaces;

	int i = 0;
	for (int j = 0; j < bookName.size(); j++)
		if (!isspace((unsigned char)bookName[j]))
			bookNameWithoutSpaces.push_back(toupper(bookName[j]));

	i = 0;
	for (int j = 0; j < searchName.size(); j++)
		if (!isspace((unsigned char)searchName[j]))
			searchNameWithoutSpaces.push_back(toupper(searchName[j]));

	if (bookNameWithoutSpaces.compare(searchNameWithoutSpaces) == 0)
		isCmp = true;
...
void SearchBookByWriter(Book* library, const int LibrarySize)
{
	...
	cout << "Введите имя и фамилию автора книги => ";
	cin.ignore(numeric_limits<streamsize>::max(), '\n');
...
void SearchBookByName(Book* library, const int LibrarySize)
{
	...
	string searchedName;
	cout << "Введите полное название книги => ";
	cin.ignore(numeric_limits<streamsize>::max(), '\n');
